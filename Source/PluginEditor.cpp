...
// Attack's parameter interval is 0.1 ms with skew 0.4. JUCE's SliderAttachment
// snaps on convertFrom0to1 and echoes that snapped value back mid-drag, which
// notches the needle around typical 0.1–20 ms values. Keep the APVTS parameter
// (range/interval/automation) unchanged; only the editor slider is continuous
// while the mouse is down.
struct PlaymakersEQAudioProcessorEditor::AttackSliderBinding : private juce::Slider::Listener
{
    AttackSliderBinding(juce::AudioProcessorValueTreeState& state,
                        const juce::String& paramID,
                        juce::Slider& sliderToBind)
        : slider(sliderToBind),
          param(*dynamic_cast<juce::RangedAudioParameter*>(state.getParameter(paramID))),
          attachment(param,
                     [this](float value)
                     {
                     ...
                     state.undoManager)
    {
        ...
        const auto nr = param.getNormalisableRange();
        slider.setNormalisableRange({ (double) nr.start, (double) nr.end, 0.0, (double) nr.skew });
    ...
    ~AttackSliderBinding() override
    {
        slider.removeListener(this);
        // These capture `this`. Leaving them on the longer-lived slider
        // makes the next range/text update a use-after-free (delete → reselect).
      ...
      presetBrowser(p.presetManager, themeManager)
{
    ...
    auto afterPresetLoad = [this]
    {
        ...
        moreButton.getProperties().set("pmAccent", hubExtrasOpen);
    ...
    dynThresholdAutoButton.getProperties().set("pmCompact", true);
    ...
    for (auto* s : { &dynThresholdSlider, &dynRangeSlider, &dynSidechainSlider })
    {
    ...
    for (auto* kn : { &dynRatioSlider, &dynAttackSlider, &dynReleaseSlider })
    {
        kn->setSliderStyle(juce::Slider::RotaryHorizontalVerticalDrag);
        kn->setTextBoxStyle(juce::Slider::TextBoxBelow, false, 54, 14);
        kn->setEnabled(false);
        kn->setSliderSnapsToMousePosition(false);
        kn->setMouseDragSensitivity(180);
    ...
    moreButton.setTooltip("Balance, slope, and Brickwall");
    hubExtrasOpen = (bool) eqProcessor.apvts.state.getProperty("hubExtrasOpen", false);
    moreButton.setToggleState(hubExtrasOpen, juce::dontSendNotification);
    moreButton.setButtonText("More");
    moreButton.getProperties().set("pmAccent", hubExtrasOpen);
    ...
    dynPanelButton.setTooltip("Enable Dynamics and edit Threshold, Comp/Exp, Ratio, Attack, and Release.");
    dynPanelButton.onClick = [this]
    {
        ...
        if (!currentlyOn)
        {
            ...
            applyDynEnabledToSelection(true);
            updatingInspector = false;
            setDynModeOpen(true);
        }
        else if (!dynPanelOpen)
        {
            setDynModeOpen(true);
        }
        else
        {
            updatingInspector = true;
            applyDynEnabledToSelection(false);
            updatingInspector = false;
            setDynModeOpen(false);
        }
        refreshInspector();
    };

    dynBackButton.setClickingTogglesState(false);
    dynBackButton.getProperties().set("pmCompact", true);
    dynBackButton.setTooltip("Return to Frequency, Gain, and Q");
    dynBackButton.onClick = [this] { setDynModeOpen(false); };
    ...
    stereoModeBox.setTooltip("Stereo / channel routing for this band");
    ...
    specFreezeButton.setTooltip("Holds the spectrum shape; peaks and valleys keep evolving slowly from live analysis");
    ...
    bandNodeHud.soloButton.onClick = [this]
    {
        if (updatingInspector)
            return;
        applySoloToSelection(bandNodeHud.soloButton.getToggleState());
        refreshInspector();
        analyzer.repaint();
    };
    ...
    bandNodeHud.removeButton.onClick = [this]
    {
        ...
        bandNodeHud.removeButton.setState(juce::Button::buttonNormal);
        refreshInspector();
    };
    ...
    secondarySheet.accentColour = floatingBandPanel.accentColour;
    ...
    floatingBandPanel.addAndMakeVisible(dynBackButton);
...
void PlaymakersEQAudioProcessorEditor::bindInspectorToBand(int bandIndex)
{
    ...
    dynAttackAttachment = std::make_unique<AttackSliderBinding>(
...
void PlaymakersEQAudioProcessorEditor::refreshInspector()
{
    ...
    removeButton.setVisible(hasSelection && !usingMetricKnobs());
    bandOptionsLabel.setVisible(hasSelection && !usingMetricKnobs());
    if (!usingMetricKnobs())
        bandEnabledButton.setVisible(hasSelection);
    else
        bandEnabledButton.setVisible(false);
    bandSoloButton.setVisible(hasSelection && !usingMetricKnobs());
    slopeLabel.setVisible(false);
    slopeSlider.setVisible(false);
    brickwallButton.setVisible(false);
    stereoLabel.setVisible(hasSelection && !usingMetricKnobs());
...
void PlaymakersEQAudioProcessorEditor::layoutBandNodeHud()
{
    ...
    const auto gain = eqProcessor.apvts.getRawParameterValue(Params::bandParamID(primary, "gain"))->load();
    const auto q = eqProcessor.apvts.getRawParameterValue(Params::bandParamID(primary, "q"))->load();
    const auto colour = Theme::bandColour(primary, t.isLight());
    const bool soloOn = eqProcessor.apvts.getRawParameterValue(Params::bandParamID(primary, "solo"))->load() >= 0.5f;

    bandNodeHud.metrics.setText(Params::formatFrequency(freq)
                                    + "   " + juce::String(gain, 1) + " dB"
                                    + "   Q " + juce::String(q, 2),
                                juce::dontSendNotification);
    bandNodeHud.metrics.setFont(Brand::uiFont(11.0f, true));
    bandNodeHud.metrics.setColour(juce::Label::textColourId, t.isLight() ? t.ink : t.softWhite);
    bandNodeHud.metrics.setColour(juce::Label::backgroundColourId, juce::Colours::transparentBlack);
    bandNodeHud.fill = t.isLight() ? t.softWhite.withAlpha(0.96f) : t.panel.withAlpha(0.94f);
    bandNodeHud.border = colour;

    const auto colourStr = colour.toString();
    bandNodeHud.soloButton.setToggleState(soloOn, juce::dontSendNotification);
    bandNodeHud.soloButton.getProperties().set("pmAccent", soloOn);
    bandNodeHud.soloButton.getProperties().set("pmAccentColour", colourStr);
    bandNodeHud.removeButton.getProperties().set("pmAccent", false);
...
void PlaymakersEQAudioProcessorEditor::timerCallback()
{
    ...
    const int primary = analyzer.getPrimarySelectedBand();
    bandList.refresh();
    if (usingMetricKnobs() && primary >= 0 && !isFloatingPanelBusy())
        layoutFloatingBandPanel(analyzer.getBounds());
    layoutBandNodeHud();
...
{
    auto attachMain = [knobs, this](juce::Component& c)
    {
        if (knobs)
    ...
    auto attachSheet = [knobs, this](juce::Component& c)
    {
        if (knobs)
            secondarySheet.addAndMakeVisible(c);
        else
            addAndMakeVisible(c);
    };
    ...
    // Keep enabled / Solo / Remove off the floating popup.
    // Solo and Remove stay available from BandNodeHud and keyboard shortcuts.
    addAndMakeVisible(bandEnabledButton);
    addAndMakeVisible(bandOptionsLabel);
    addAndMakeVisible(bandSoloButton);
...
void PlaymakersEQAudioProcessorEditor::layoutFloatingBandPanel(juce::Rectangle<int> graphBounds)
{
    ...
    const int knobsH = 84;
    const int headH = 16;
    const int knobsGap = 2;
    const int dynRowH = 22;
    const int dynKnobRowH = 74;
    const int dynFooterH = 16;
    const bool dynOn = canDyn
        && eqProcessor.apvts.getRawParameterValue(Params::bandParamID(primary, "dynEnabled"))->load() >= 0.5f;
    if (dynPanelOpen && (!canDyn || !dynOn))
        dynPanelOpen = false;
    const bool dynMode = dynPanelOpen && canDyn && dynOn;
    if (isFloatingPanelBusy())
        return;
    ...
    const int targetW = 308;
    const int panelW = juce::jmin(targetW, juce::jmax(276, plot.getWidth() - 16));
    const int panelH = dynMode
        ? (padY * 2 + headH + 3 + dynRowH + 3 + dynRowH + 2 + dynKnobRowH + 2 + dynFooterH)
        : (padY * 2 + headH + knobsGap + knobsH);
    ...
    popupCloseButton.setBounds(head.removeFromRight(16).withHeight(15).withY(head.getY() + 1));
    head.removeFromRight(3);
    moreButton.setVisible(true);
    moreButton.setButtonText("More");
    moreButton.setToggleState(hubExtrasOpen, juce::dontSendNotification);
    moreButton.getProperties().set("pmAccent", hubExtrasOpen);
    moreButton.setBounds(head.removeFromRight(40).withHeight(15).withY(head.getY() + 1));
    head.removeFromRight(6);
    stereoLabel.setVisible(false);
    stereoLabel.setBounds({});
    stereoModeBox.setVisible(true);
    stereoModeBox.setBounds(head.removeFromRight(90).withHeight(15).withY(head.getY() + 1));
    metricModeButton.setVisible(false);
    metricModeButton.setBounds({});

    applyFloatingExtrasVisibility(hubExtrasOpen);
    ...
    if (dynMode)
    {
        ...
        dynThresholdSlider.setEnabled(!autoTrack);
        dynThresholdAutoButton.setVisible(true);
        dynThresholdAutoButton.setEnabled(true);
        dynAutoThresholdButton.setVisible(true);
        dynAutoThresholdButton.setEnabled(true);
        for (auto* l : { &dynThresholdLabel, &dynRangeLabel, &dynRatioLabel, &dynAttackLabel, &dynReleaseLabel })
            l->setVisible(true);
        updateDynRangeLabelForBand(primary);

        const int rowLabelW = 42;
        const int rowCtrlH = 16;
        auto alignRowCtrl = [](juce::Rectangle<int>& row, int w, int h)
        {
            auto cell = row.removeFromLeft(w);
            return cell.withHeight(h).withY(cell.getY() + (cell.getHeight() - h) / 2);
        };

        auto threshRow = r.removeFromTop(dynRowH);
        dynThresholdLabel.setBounds(threshRow.removeFromLeft(rowLabelW).withTrimmedTop(3));
        threshRow.removeFromLeft(6);
        dynThresholdAutoButton.setBounds(alignRowCtrl(threshRow, 38, rowCtrlH));
        threshRow.removeFromLeft(6);
        dynAutoThresholdButton.setBounds(alignRowCtrl(threshRow, 58, rowCtrlH));
        threshRow.removeFromLeft(8);
        dynThresholdSlider.setTextBoxStyle(juce::Slider::TextBoxRight, false, 48, 16);
        dynThresholdSlider.setBounds(threshRow.withHeight(rowCtrlH).withY(threshRow.getY() + (threshRow.getHeight() - rowCtrlH) / 2));

        r.removeFromTop(3);
        auto compRow = r.removeFromTop(dynRowH);
        dynRangeLabel.setBounds(compRow.removeFromLeft(rowLabelW).withTrimmedTop(3));
        dynSidechainButton.setVisible(true);
        dynSidechainButton.setEnabled(true);
        dynSidechainLabel.setVisible(true);
        dynSidechainButton.setBounds(compRow.removeFromRight(36).withHeight(rowCtrlH)
                                         .withY(compRow.getY() + (compRow.getHeight() - rowCtrlH) / 2));
        compRow.removeFromRight(4);
        dynSidechainLabel.setBounds(compRow.removeFromRight(52).withTrimmedTop(3));
        compRow.removeFromRight(8);
        dynRangeSlider.setTextBoxStyle(juce::Slider::TextBoxRight, false, 48, 16);
        dynRangeSlider.setBounds(compRow.withHeight(rowCtrlH).withY(compRow.getY() + (compRow.getHeight() - rowCtrlH) / 2));
        updateDynSidechainButton();

        r.removeFromTop(2);
        auto knobRow = r.removeFromTop(dynKnobRowH);
        const int gutter = 8;
        const int col = juce::jmax(80, (knobRow.getWidth() - 2 * gutter) / 3);
        auto placeDynKnob = [](juce::Rectangle<int> cell, juce::Label& l, juce::Slider& s)
        {
            auto inner = cell.reduced(2, 0);
            l.setBounds(inner.removeFromTop(11));
            l.setJustificationType(juce::Justification::centred);
            s.setSliderStyle(juce::Slider::RotaryHorizontalVerticalDrag);
            s.setTextBoxStyle(juce::Slider::TextBoxBelow, false, 54, 14);
            s.getProperties().set("pmLargeKnob", false);
            s.setBounds(inner);
        };
        placeDynKnob(knobRow.removeFromLeft(col), dynRatioLabel, dynRatioSlider);
        knobRow.removeFromLeft(gutter);
        placeDynKnob(knobRow.removeFromLeft(col), dynAttackLabel, dynAttackSlider);
        knobRow.removeFromLeft(gutter);
        placeDynKnob(knobRow, dynReleaseLabel, dynReleaseSlider);

        r.removeFromTop(2);
        auto footer = r.removeFromTop(dynFooterH);
        dynSectionLabel.setVisible(false);
        dynSectionLabel.setBounds({});
        dynPanelButton.setVisible(true);
        dynBackButton.setVisible(true);
        dynPanelButton.setBounds(footer.removeFromLeft(36).withHeight(dynFooterH));
        dynBackButton.setBounds(footer.removeFromRight(40).withHeight(dynFooterH));
        dynPanelButton.toFront(false);
        dynBackButton.toFront(false);
    }
    else
    {
        hideDynModeControls();
        dynBackButton.setVisible(false);
        dynBackButton.setBounds({});
        ...
        const int sideW = juce::jmax(80, knobsRow.getWidth() * 28 / 100);
        const int centerW = juce::jmax(100, knobsRow.getWidth() - sideW * 2 - knobGutter * 2);
...
void PlaymakersEQAudioProcessorEditor::setSecondarySheetOpen(bool open)
{
    ...
    moreButton.getProperties().set("pmAccent", open);
    moreButton.setButtonText("More");
    moreButton.repaint();
...
void PlaymakersEQAudioProcessorEditor::hideDynModeControls()
{
    dynBackButton.setVisible(false);
    dynBackButton.setBounds({});
    dynSectionLabel.setVisible(false);
    ...
    for (auto* c : std::initializer_list<juce::Component*> {
            ...
            &dynRangeLabel, &dynRangeSlider, &dynRatioLabel, &dynRatioSlider,
            &dynAttackLabel, &dynAttackSlider, &dynReleaseLabel, &dynReleaseSlider,
            &dynSidechainLabel, &dynSidechainButton })
    {
        c->setVisible(false);
        c->setBounds({});
    }
...
bool PlaymakersEQAudioProcessorEditor::isFloatingPanelBusy() const
{
    return freqKnob.isMouseButtonDown()
        ...
        || dynAttackSlider.isMouseButtonDown()
        || dynReleaseSlider.isMouseButtonDown();
}

bool PlaymakersEQAudioProcessorEditor::isSecondarySheetBusy() const
{
    return slopeSlider.isMouseButtonDown()
        || balanceSlider.isMouseButtonDown();
}

void PlaymakersEQAudioProcessorEditor::hideSecondarySheet()
{
    secondarySheet.setVisible(false);
    if (!usingMetricKnobs())
        return;
...
{
    const int primary = analyzer.getPrimarySelectedBand();
    if (primary < 0 || !usingMetricKnobs() || !hubExtrasOpen)
    {
        hideSecondarySheet();
        return;
    }
    ...
    const int padY = 8;
    const int rowH = 22;
    const int extrasRows = 1 + (canSlope ? 1 : 0);
    const int sheetH = padY * 2 + extrasRows * rowH + (extrasRows - 1) * 4;
    const int sheetW = 308;

    if (!isSecondarySheetBusy())
    {
        layoutBandNodeHud();
        ...
        const auto usable = plot.getWidth() > 16 && plot.getHeight() > 16
            ? plot.reduced(6)
            : plot;
        ...
        auto clampToUsable = [&](juce::Rectangle<int> s)
        {
            if (usable.getWidth() >= sheetW)
                s.setX(juce::jlimit(usable.getX(), usable.getRight() - sheetW, s.getX()));
            else
                s.setX(usable.getX());
        ...
        auto makeAt = [&](int x, int y)
        {
            return clampToUsable({ x, y, sheetW, sheetH });
        };

        const int preferredX = panel.getCentreX() - sheetW / 2;
        const int aboveY = panel.getY() - gap - sheetH;
        const int belowY = panel.getBottom() + gap;
        const int sideY = juce::jlimit(usable.getY(),
                                       juce::jmax(usable.getY(), usable.getBottom() - sheetH),
                                       panel.getY());

        juce::Array<juce::Rectangle<int>> candidates;
        candidates.add(makeAt(preferredX, aboveY));
        candidates.add(makeAt(avoid.getX() - gap - sheetW, aboveY));
        candidates.add(makeAt(avoid.getRight() + gap, aboveY));
        candidates.add(makeAt(panel.getRight() + gap, sideY));
        candidates.add(makeAt(panel.getX() - gap - sheetW, sideY));
        candidates.add(makeAt(preferredX, belowY));
        candidates.add(makeAt(avoid.getX() - gap - sheetW, belowY));
        candidates.add(makeAt(avoid.getRight() + gap, belowY));

        auto scoreOf = [&](juce::Rectangle<int> s) -> int
        {
            int score = 0;
            if (s.intersects(avoid))
                score += 1000 + s.getIntersection(avoid).getWidth() * s.getIntersection(avoid).getHeight();
            if (!usable.contains(s) && !usable.intersects(s))
                score += 800;
            if (s.intersects(panel))
                score += 200;
            if (s.getBottom() <= panel.getY())
                score += 0;
            else if (s.getY() >= panel.getBottom())
                score += 40;
            else
                score += 20;
            score += std::abs(s.getX() - preferredX) / 8;
            return score;
        };

        auto best = candidates.getFirst();
        int bestScore = scoreOf(best);
        for (int i = 1; i < candidates.size(); ++i)
        {
            const int score = scoreOf(candidates.getReference(i));
            if (score < bestScore)
            {
                bestScore = score;
                best = candidates.getReference(i);
            }
        }

        secondarySheet.setBounds(best);
    }

    secondarySheet.setVisible(true);
    secondarySheet.toFront(false);

    auto r = secondarySheet.getLocalBounds().reduced(padX, padY);

    auto tools = r.removeFromTop(rowH);
    balanceLabel.setVisible(true);
    balanceSlider.setVisible(true);
    balanceLabel.setBounds(tools.removeFromLeft(28).withTrimmedTop(4));
    tools.removeFromLeft(4);
    balanceSlider.setBounds(tools.withHeight(16).withY(tools.getY() + 3));

    if (canSlope)
    {
        r.removeFromTop(4);
        auto opt = r.removeFromTop(rowH);
        slopeLabel.setVisible(true);
        slopeSlider.setVisible(true);
        slopeSlider.setEnabled(true);
        brickwallButton.setVisible(true);
        brickwallButton.setEnabled(true);
        slopeLabel.setBounds(opt.removeFromLeft(40).withTrimmedTop(4));
        slopeSlider.setBounds(opt.removeFromLeft(120));
        opt.removeFromLeft(8);
        brickwallButton.setBounds(opt.removeFromLeft(78).withHeight(16).withY(opt.getY() + 3));
    }
    else
    {
        slopeLabel.setVisible(false);
...
void PlaymakersEQAudioProcessorEditor::updateDynPanelButton()
{
    ...
    dynPanelButton.setTooltip(canDyn
        ? "Enable Dynamics and edit Threshold, Comp/Exp, Ratio, Attack, and Release."
...
void PlaymakersEQAudioProcessorEditor::updateMetricModeVisibility(bool hasSelection)
{
    ...
    if (knobs)
    {
        // Band chrome lives in the floating panel; slope/balance stay on the More sheet.
        bandOptionsLabel.setVisible(false);
...
void PlaymakersEQAudioProcessorEditor::applyThemeToButtons()
{
    ...
    for (auto* s : { &dynThresholdSlider, &dynRangeSlider, &dynSidechainSlider, &slopeSlider, &outputGainSlider })
        s->setTextBoxStyle(juce::Slider::TextBoxRight, false, 44, 20);
    for (auto* kn : { &dynRatioSlider, &dynAttackSlider, &dynReleaseSlider })
        kn->setTextBoxStyle(juce::Slider::TextBoxBelow, false, 54, 14);
    for (auto* kn : { &freqKnob, &gainKnob, &qKnob })
...
void PlaymakersEQAudioProcessorEditor::applyThemeToInspector()
{
    ...
    prepCaption(dynRatioLabel);
    prepCaption(dynAttackLabel);
    prepCaption(dynReleaseLabel);
    dynRatioLabel.setJustificationType(juce::Justification::centred);
    ...
    for (auto* kn : { &freqKnob, &gainKnob, &qKnob, &dynRatioSlider, &dynAttackSlider, &dynReleaseSlider })
    {
...
void PlaymakersEQAudioProcessorEditor::resized()
{
    ...
    if (dynThresholdSlider.isVisible())
    {
        ...
        for (auto* kn : { &dynRatioSlider, &dynAttackSlider, &dynReleaseSlider })
        {
            kn->setSliderStyle(juce::Slider::LinearHorizontal);
            kn->setTextBoxStyle(juce::Slider::TextBoxRight, false, 44, 20);
        }
